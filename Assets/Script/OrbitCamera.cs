...
{
#if UNITY_EDITOR
    [Header("Debug elements"), Space]
    [SerializeField] Color gizmosColor = Color.green;

    [Space]
#endif

    [Header("Class Variables"), Space]
    ...
    private void OnValidate()
    {
        ...
        if (focus != null)
        {
            focusPoint = focus.position + (focus.up * heighOffset);
        }
    }
    ...
    void LateUpdate ()
    {
        ...
        Quaternion lookRotation = _gravityAlignment * orbitRotation;
        ...
        Vector3 castFrom = focus.position + (focus.up * heighOffset);
    ...
    private void OnDrawGizmos()
    {
        if (focus != null)
        {
            ...
            Handles.DrawDottedLine(transform.position, focusPoint, .5f);
            Handles.DrawWireDisc(focusPoint, focus.up, focusRadius);
        }
    }
#endif
    ...
    void UpdateFocusPoint()
    {
        ...
        Vector3 targetPoint = focus.position + (focus.up * heighOffset);
    ...
    bool AutomaticRotation () {
        ...
        Vector3 alignedDelta = Quaternion.Inverse(_gravityAlignment) * (focusPoint - previousFocusPoint);
