...
public class CustomGravityRigidbody : MonoBehaviour
{
    ...
    [SerializeField, Min(0f)] float buoyancy = 1.1f;
    ...
    void FixedUpdate () {
        ...
        float buyancyFactor = -buoyancy / buoyancyOffsets.Length;
        ...
        for (int i = 0; i < buoyancyOffsets.Length; i++)
        {
            if (submergence[i] > 0f) {
                ...
                body.AddForceAtPosition(gravity * (buyancyFactor * submergence[i]), transform.TransformPoint(buoyancyOffsets[i]), ForceMode.Acceleration);
